...
def saveExr(path, img):
	...
	io.imwrite(path, img, format='EXR')
...
def average(paths):
	assert len(paths) > 0
	res = readExr(paths[0]).astype('float64')
	for i in range(1, len(paths)):
		res += readExr(paths[i]).astype('float64')
	res /= len(paths)
	return res
				...
				for exec_filter in exec_filters:
					for sampler in samplers:
						...
						for it in range(number_of_iterations):
							...
							name = img_name + '_gt_' + str(it)
							imgname = name + '.exr'
							filenames.append(imgname)
							
							img_path = result_folder + sub_folder + imgname
							img_paths.append(img_path)

							if not os.path.exists(result_folder + sub_folder):
								os.makedirs(result_folder + sub_folder)				
							
							command = [pbrt_exe, pbrt_scene.tmp_filename, '--outfile', img_path, '--nthreads', str(num_threads)]
